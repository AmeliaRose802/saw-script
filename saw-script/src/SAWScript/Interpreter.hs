...
primitives = Map.fromList $
 ...
 , prim "enable_crucible_assert_then_assume" "TopLevel ()"
  ...
  , prim "llvm_bind_method"
    ...
    , "(from 'llvm_alloc'). The second argument is the vtable slot index"
    , "(zero-based). The third argument is a proved specification (from"
    , "'llvm_verify' or 'llvm_unsafe_assume_spec') to use as the override"
    , "for the virtual method at that slot."
    , ""
    , "During simulation, the vtable memory chain is set up so that"
